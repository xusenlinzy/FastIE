...
def set2json(labels: Set[Tuple[str, str, str]]) -> List[Triple]:
    return [Triple(subject=_sub, object=_obj, relation=_type) for _type, _sub, _obj in labels]
...
class RelExtractionDecoder(nn.Module):
    ...
    def _auto_joiner(
        self,
        short_results: List[Set[Tuple[str, str, str]]],
    ...
    def start_multi_process_pool(self, target_devices: List[str] = None) -> Dict[Literal["input", "output", "processes"], Any]:
        ...
        for device_id in target_devices:
            p = ctx.Process(
                target=RelExtractionDecoder._predict_multi_process_worker,
    ...
    def _predict_multi_process_worker(
        target_device: str, model: "RelExtractionDecoder", input_queue: Queue, results_queue: Queue
    ) -> None:
        """
